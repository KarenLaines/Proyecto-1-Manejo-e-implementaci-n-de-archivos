...
class GIFInfoApp:
    def __init__(self, master):
        ...
        style.configure("TLabel", background="#191970", foreground="white", font=("Helvetica", 10))
        style.configure("TButton", background="white", foreground="indian red", font=("Helvetica", 10, "bold"), padding=8)
        style.map("TButton", background=[("active", "slategray2")], foreground=[("active", "white")])
        ...
        self.open_button = ttk.Button(button_frame, text="Abrir Carpeta", command=self.load_gifs_from_folder, style="TButton")
        self.open_button.grid(row=0, column=0, padx=10, pady=5, ipadx=10, ipady=5)

        self.load_button = ttk.Button(button_frame, text="Cargar Datos desde JSON", command=self.load_data_from_json, style="TButton")
        self.load_button.grid(row=0, column=1, padx=10, pady=5, ipadx=10, ipady=5)

        self.save_button = ttk.Button(button_frame, text="Guardar Cambios", command=self.save_changes, style="TButton")
        self.save_button.grid(row=0, column=2, padx=10, pady=5, ipadx=10, ipady=5)

        self.close_button = ttk.Button(button_frame, text="Salir", command=master.quit, style="TButton")
    ...
    def display_gif_info_from_folder(self, folder_path):
        try:
            ...
            # Función recursiva para obtener todos los archivos GIF en subcarpetas
            gif_files = []
            ...
            if not gif_files:
                messagebox.showinfo("Información", "No se encontraron archivos GIF en la carpeta seleccionada.")
                return

            for file_path in gif_files:
                gif_info = read_gif_info(file_path)
                gif_info["Carpeta"] = os.path.dirname(file_path)  # Agrega la ruta donde se encontró el GIF
    ...
    def display_gif_info(self, file_path, gif_info):
        ...
        edit_button = ttk.Button(frame, text="Editar Información", command=lambda: self.edit_gif_info(listbox, gif_info, file_path), style="TButton")
    ...
    def edit_gif_info(self, listbox, gif_info, file_path):
        ...
        if not selected_index:
            messagebox.showinfo("Información", "Por favor, selecciona un elemento de la lista para editar.")
            return
        ...
        if ": " in selected_line:
            ...
            new_value = simpledialog.askstring("Editar Información", f"Editar {key}", initialvalue=value)
